...
public class LibraryHelper extends DatabaseHelper {
    ...
    public boolean updatePatron(ArrayList<String> values,boolean pswd) throws SQLException {
        String sql = null;
        if(pswd){
...
//        return preSql.toString
    ...
    public boolean updateAdmin(ArrayList<String> values,boolean pswd) throws SQLException {
        String sql = null;
        if(pswd){
...
//        return preSql.toString
    ...
    public ResultSet fetchBorrowedBooks(String id) throws SQLException {
        sql = "SELECT * from borrower inner join books on borrower.book_Id = books.books_Id where borrower.patron_Id = ? and returned is NULL";
    ...
    public ResultSet fetchHistoryOf(String id) throws SQLException {
        sql = "SELECT borrower.*,patron.patron_Name,admin.admin_Name,books.book_Name FROM borrower JOIN patron ON patron.patron_Id = borrower.patron_Id JOIN admin ON admin.admin_Id = borrower.admin_Id JOIN books ON borrower.book_Id = books.books_Id where borrower.returned IS NOT NULL AND borrower.patron_Id = ?";
    ...
    public ResultSet fetchAllGenres() throws SQLException {
...
//        System.out.println(sql);
    ...
    public ResultSet fetchAllAuthors() throws SQLException {
...
//        System.out.println(sql);
       ...
       public boolean updatePassword(String id,String value) throws SQLException {
        String sql = null;
        sql = "UPDATE patron SET patron_Hash2 = ? where patron_Id = ?";
        preSql = conn.prepareStatement(sql);
        preSql.setString(1, value);
...
//        return preSql.toString
        ...
        public ResultSet checkExists(String table,String name) throws SQLException {
        sql = "SELECT EXISTS(SELECT * from "+table+" WHERE "+table+"_Name="+name+") as status";
//        System.out.println(sql);
        preSql = conn.prepareStatement(sql);
        System.out.println(preSql);
        return preSql.executeQuery();
